...
import { AbbyConnectorError, parseAbbyCredential } from "@/lib/connectors/abby/client";
import { runAbbySync, saveAbbyCredential } from "@/lib/connectors/abby/sync-service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const credential = parseAbbyCredential({ apiKey });
    const { accountLabel } = await saveAbbyCredential(user.id, projectId, credential);
    ...
    const message = err instanceof Error ? err.message : "Erreur connecteur Abby";
    return NextResponse.json(
      { error: message },
