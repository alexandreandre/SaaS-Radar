import { NextResponse } from "next/server";
import { getCurrentUser } from "@/lib/auth";
import { assertProjectOwnedByUser } from "@/lib/connectors/project-access";
import {
  AbbyConnectorError,
  parseAbbyCredential,
  validateCredential,
} from "@/lib/connectors/abby/client";
import { parseAbbyApiKey } from "@/lib/connectors/abby/keys";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

export async function POST(request: Request) {
  const user = await getCurrentUser();
  if (!user) {
    return NextResponse.json({ error: "Non authentifié" }, { status: 401 });
  }

  let body: unknown;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ error: "JSON invalide" }, { status: 400 });
  }

  const b = body as Record<string, unknown>;
  const projectId = typeof b.projectId === "string" ? b.projectId.trim() : "";
  const apiKey = typeof b.apiKey === "string" ? b.apiKey.trim() : "";

  if (!projectId) {
    return NextResponse.json({ error: "projectId requis" }, { status: 400 });
  }
  if (!apiKey) {
    return NextResponse.json({ error: "apiKey requis" }, { status: 400 });
  }

  try {
    await assertProjectOwnedByUser(user.id, projectId);
    const parsedKey = parseAbbyApiKey(apiKey);
    const credential = parseAbbyCredential({ apiKey: parsedKey });
    const meta = await validateCredential(credential);

    return NextResponse.json({
      accountLabel: meta.accountLabel,
      companyName: meta.commercialName,
      companyId: meta.companyId,
    });
  } catch (err) {
    const status = err instanceof AbbyConnectorError && err.status ? err.status : 500;
    const message = err instanceof Error ? err.message : "Validation Abby échouée";
    return NextResponse.json(
      { error: message },
      { status: status >= 400 && status < 600 ? status : 500 },
    );
  }
}
