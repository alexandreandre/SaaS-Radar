...
export async function GET(request: Request) {
  ...
  if (!isLinkedInAdsConfigured()) {
    return NextResponse.json(
      {
        error:
          "LinkedIn Ads OAuth non configuré (LINKEDIN_ADS_CLIENT_ID, LINKEDIN_ADS_CLIENT_SECRET, LINKEDIN_ADS_REDIRECT_URI)",
        configured: false,
      },
      { status: 503 },
    );
  ...
  return NextResponse.redirect(getLinkedInAdsAuthorizeUrl(state));
}
