...
import {
  ...
  LinkedInAdsConnectorError,
  listAccessibleAdAccounts,
} from "@/lib/connectors/linkedin-ads/client";
import {
  loadLinkedInAdsCredential,
  saveLinkedInAdsCredentialState,
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!credential?.accessToken) {
      return NextResponse.json(
        { error: "LinkedIn Ads non autorisé — connectez-vous via OAuth d'abord" },
        { status: 400 },
      );
    }
    ...
    if (refreshed) {
      await saveLinkedInAdsCredentialState(user.id, projectId, freshCredential);
    }
