...
import {
  ...
  MetaAdsConnectorError,
  listAccessibleAdAccounts,
} from "@/lib/connectors/meta-ads/client";
import {
  loadMetaAdsCredential,
  saveMetaAdsCredentialState,
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const credential = await loadMetaAdsCredential(user.id, projectId);
    if (!credential?.accessToken) {
      return NextResponse.json(
        { error: "Meta Ads non autorisé — connectez-vous via OAuth d'abord" },
        { status: 400 },
      );
    }
    ...
    if (refreshed) {
      await saveMetaAdsCredentialState(user.id, projectId, freshCredential);
    }
    const accounts = await listAccessibleAdAccounts(freshCredential);
