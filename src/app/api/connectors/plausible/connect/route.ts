...
import { runPlausibleSync, savePlausibleCredential } from "@/lib/connectors/plausible/sync-service";
...
export async function POST(request: Request) {
  ...
  const signupGoalDisplayName =
    typeof b.signupGoalDisplayName === "string" ? b.signupGoalDisplayName.trim() : null;
  ...
  try {
    ...
    const credential = parsePlausibleCredential({
      apiKey,
      siteId,
      signupGoalDisplayName: signupGoalDisplayName || null,
    });
    const { accountLabel } = await savePlausibleCredential(user.id, projectId, credential);
    ...
    const message = err instanceof Error ? err.message : "Erreur connecteur Plausible";
    return NextResponse.json(
      { error: message },
