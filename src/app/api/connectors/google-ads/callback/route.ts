...
export async function GET(request: Request) {
  ...
  if (oauthError) {
    return NextResponse.redirect(new URL("/mes-saas?google_ads=denied", request.url));
  }

  if (!code || !stateRaw) {
    return NextResponse.redirect(new URL("/mes-saas?google_ads=error", request.url));
  }
  ...
  try {
    ...
    if (state.userId !== user.id) {
      return NextResponse.redirect(new URL("/mes-saas?google_ads=unauthorized", request.url));
    }
    projectId = state.projectId ?? null;
  } catch {
    return NextResponse.redirect(new URL("/mes-saas?google_ads=invalid_state", request.url));
  }

  if (!projectId) {
    return NextResponse.redirect(new URL("/mes-saas?google_ads=missing_project", request.url));
  }
  ...
  if (!isCredentialsEncryptionConfigured()) {
    ...
    cockpitUrl.searchParams.set("google_ads_oauth", "encryption_error");
  ...
  const cockpitUrl = new URL(`/cockpit/${projectId}`, request.url);
  cockpitUrl.searchParams.set("module", "integrations");
  cockpitUrl.searchParams.set("google_ads_oauth", "1");
  return NextResponse.redirect(cockpitUrl);
}
