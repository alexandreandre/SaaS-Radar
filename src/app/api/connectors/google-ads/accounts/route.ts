...
import {
  loadGoogleAdsCredential,
  saveGoogleAdsCredentialState,
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const credential = await loadGoogleAdsCredential(user.id, projectId);
    if (!credential?.refreshToken) {
      return NextResponse.json(
        { error: "Google Ads non autorisé — connectez-vous via OAuth d'abord" },
        { status: 400 },
      );
    }
    ...
    if (refreshed) {
      await saveGoogleAdsCredentialState(user.id, projectId, freshCredential);
    }
