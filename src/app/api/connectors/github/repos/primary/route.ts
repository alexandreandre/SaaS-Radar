...
import { setPrimaryGitHubRepo } from "@/lib/connectors/github/sync-service";
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const credential = await setPrimaryGitHubRepo(user.id, projectId, repoFullName);
    ...
    const message = err instanceof Error ? err.message : "Erreur définition repo principal";
    return NextResponse.json(
      { error: message },
