...
import { HubSpotConnectorError } from "@/lib/connectors/hubspot/client";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const sync = await runHubSpotSync(user.id, projectId);
    ...
    const status = err instanceof HubSpotConnectorError && err.status ? err.status : 500;
    const message = err instanceof Error ? err.message : "Erreur HubSpot";
    return NextResponse.json(
      { error: message },
