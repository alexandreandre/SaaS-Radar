...
import { runCrispSync, saveCrispCredential } from "@/lib/connectors/crisp/sync-service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { accountLabel } = await saveCrispCredential(user.id, projectId, credential);
    const sync = await runCrispSync(user.id, projectId);
    ...
    const message = err instanceof Error ? err.message : "Erreur connecteur Crisp";
    return NextResponse.json(
      { error: message },
