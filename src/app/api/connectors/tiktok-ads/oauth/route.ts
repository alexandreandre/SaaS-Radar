...
export async function GET(request: Request) {
  ...
  if (!isTikTokAdsConfigured()) {
    return NextResponse.json(
      {
        error:
          "TikTok Ads OAuth non configuré (TIKTOK_ADS_APP_ID, TIKTOK_ADS_APP_SECRET, TIKTOK_ADS_REDIRECT_URI)",
        configured: false,
      },
      { status: 503 },
    );
  ...
  return NextResponse.redirect(getTikTokAdsAuthorizeUrl(state));
}
