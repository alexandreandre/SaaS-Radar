  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onExplore}
        className="group w-full rounded-2xl border border-white/15 bg-[#0A0A0A]/95 p-4 text-left shadow-2xl shadow-black/50 backdrop-blur-xl transition-all hover:border-accent/50"
      >
        <div className="flex items-start justify-between gap-2">
          <div>
            <span className="text-2xl">{market.flag}</span>
            <h3 className="mt-1 text-base font-semibold text-white">{market.name}</h3>
            <p className="text-[10px] text-zinc-500">Origine · export vers {target.flag} {target.name}</p>
          </div>
          <div className="flex flex-col items-end gap-1">
            <ScopeBadge scope={market.scope} targetName={target.name} />
            <span className="text-[10px] tabular-nums text-zinc-400">Heat {market.heatScore}</span>
          </div>
        </div>
        ...
        <div className="mt-3 rounded-lg border border-white/10 bg-white/5 px-3 py-2">
          ...
          <p className={cn("mt-0.5 text-sm font-semibold", fit.score >= 65 ? "text-emerald-400" : "text-zinc-400")}>
        ...
        {topEarner && (
          <div className="mt-3 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
            <p className="text-[10px] font-medium uppercase tracking-wider text-zinc-500">Top revenu</p>
            <div className="mt-1 flex items-center justify-between gap-2">
              <p className="truncate text-sm font-medium text-white">{topEarner.name}</p>
              <p className="shrink-0 text-sm font-semibold text-accent">{topEarner.mrrLabel}</p>
            </div>
            {adaptableToTarget && (
        ...
        <div className="mt-4 flex items-center justify-between rounded-lg bg-accent px-3 py-2 text-sm font-medium text-white group-hover:bg-blue-600">
...
function ScopeBadge({ scope, targetName }: { scope: WorldMarket["scope"]; targetName: string }) {
  return (
    <span className="rounded-full border border-white/15 bg-white/10 px-2 py-0.5 text-[10px] font-medium text-zinc-300">
...
function MiniStat({
  ...
  return (
    <div className="rounded-lg bg-white/5 px-2 py-1.5 text-center">
      <p className={cn("text-sm font-semibold tabular-nums", accent ? "text-accent" : "text-white")}>{value}</p>
