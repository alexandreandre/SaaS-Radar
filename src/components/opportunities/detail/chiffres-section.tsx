...
function TractionSource({ signal }: { signal?: TractionSignal }) {
  ...
  if (signal.sourceUrl) {
    return (
      <a
        ...
        className="mt-1 flex items-center justify-end gap-1 text-xs text-gray-600 transition-colors hover:text-blue-400"
      >
  ...
  return <p className="mt-1 text-xs text-gray-600">Source : {signal.source}</p>;
}
