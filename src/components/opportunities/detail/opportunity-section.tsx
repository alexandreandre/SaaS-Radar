      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-border">
        <div className="flex items-center justify-between border-b border-border bg-muted/40 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-foreground">{opportunity.originCountry}</p>
              <p className="mt-0.5 text-xs capitalize text-muted-foreground">{opportunity.sector}</p>
            </div>
          </div>
          ...
          <a
            ...
            className="flex items-center gap-1.5 rounded-full border border-primary/30 bg-primary/20 px-3 py-1.5 text-xs text-blue-400 transition-colors hover:text-blue-300"
          >
        ...
        <div className="divide-y divide-border/60">
          {rows.map((row, i) => (
            <div
              key={i}
              className="flex items-center justify-between px-6 py-4 transition-colors hover:bg-muted/30/30"
            >
              <p className="text-sm text-muted-foreground">{row.label}</p>
              {row.type === "text" ? (
                <p className="text-sm font-medium capitalize text-foreground">{row.value}</p>
              ) : (
                <span
                  className={cn(
