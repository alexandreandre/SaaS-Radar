...
import { resolveChannelKey, type AcquisitionChannelKey } from "@/lib/acquisition-channels";
...
function buildContext(opportunity: Opportunity, channel: string): ContentContext {
  ...
  return { productName, target, price, pain, sector: opportunity.sector, tactics };
}

function emailFromTemplates(opportunity: Opportunity, channelKey: AcquisitionChannelKey): string | null {
...
export function generateAcquisitionContent(
  ...
  const ctx = buildContext(opportunity, channel);
